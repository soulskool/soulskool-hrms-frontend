...
//                                 <button
//   onClick={loadMoreHistory}
//   disabled={history.loadingMore || !hasMore}
//   className="btn-secondary text-sm flex items-center justify-center mx-auto disabled:opacity-50"
// >
//   {history.loadingMore ? <Loader2 className="animate-spin mr-2" size={16}/> : <ChevronDown size={16} className="mr-1"/>}
//   {history.loadingMore ? 'Loading...' : (hasMore ? 'Load Older Records' : 'No older records')}
// </button>
//                              </div>
//                         </div>
//                     ) : (
...
import { fetchTodayOverview, fetchAttendanceHistory } from "../../../lib/redux/slices/adminAttendanceSlice";
import StatCard from "@/components/admin/attendancePage/StatCard";
import TodayCheckinsTable from "@/components/admin/attendancePage/TodayCheckinsTable";
import AttendanceHistoryTable from "@/components/admin/attendancePage/AttendanceHistoryTable";
import { Users, LogIn, UserX, Clock, AlertCircle } from "lucide-react";
import ExportMonthlyAttendance from "@/components/admin/attendancePage/ExportMonthlyAttendance";
...
export default function AttendanceOverviewPage() {
  ...
  const { todayOverview, history, loadingOverview, loadingHistory, error } = useSelector(
  ...
  useEffect(() => {
    dispatch(fetchTodayOverview());
    dispatch(fetchAttendanceHistory(1));
  }, [dispatch]);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Error (only for real failures while more pages are expected) */}
      {showError && (
        <div className="flex items-center p-3 text-sm text-red-700 bg-red-100 rounded-md space-x-2">
          <AlertCircle size={18} />
          <span>Error loading data: {error}</span>
        ...
        {loadingOverview ? (
          <div className="flex justify-center items-center h-24">
            <div className="animate-spin text-blue-600">
              {/* lightweight spinner placeholder */}
            </div>
          </div>
        ) : todayOverview ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <TodayCheckinsTable
        ...
        rows={todayOverview?.checkedInList ?? []}
      ...
      <AttendanceHistoryTable
        ...
        data={history.data}
        formatHistoryDateHeader={formatHistoryDateHeader}
